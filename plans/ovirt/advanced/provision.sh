...
ovirt-shell -E "add host --address rhvnode1.default --cluster-name Default --name rhvnode1.default --root_password $PASSWORD"
ovirt-shell -E "add host --address rhvnode2.default --cluster-name Default --name rhvnode2.default --root_password $PASSWORD"
sleep 240
ovirt-shell -E "add storagedomain --name vms --host-name rhvnode1.default --type data --storage-type nfs --storage-address rhvengine.default --storage-path /vms"
