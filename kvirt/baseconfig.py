...
class Kbaseconfig:
        ...
        if not os.path.exists(confpoolfile):
            ...
            with open(confpoolfile, 'r') as entries:
                ...
                if self.confpools is None:
                    self.confpools = {}
        self.clusterconfigs = {}
        clusterconfigsdir = f"{os.path.dirname(sys.modules[Kbaseconfig.__module__].__file__)}/clusterconfigs"
        for clusterconfig in os.listdir(clusterconfigsdir):
            entry = clusterconfig.replace('.yml', '')
            self.clusterconfigs[entry] = yaml.safe_load(open(f"{clusterconfigsdir}/{clusterconfig}"))
        clusterconfigsfile = default.get('clusterconfigs', f"{os.environ.get('HOME')}/.kcli/clusterconfigs.yml")
        clusterconfigsfile = os.path.expanduser(clusterconfigsfile)
        if os.path.exists(clusterconfigsfile):
            with open(clusterconfigsfile, 'r') as entries:
                try:
                    self.clusterconfigs.update(yaml.safe_load(entries))
                except yaml.scanner.ScannerError as e:
                    error(f"Couldn't parse yaml in .kcli/clusterconfigs.yml. Hit {e}")
                    sys.exit(1)
        self.extraclients = {}
        self._extraclients = []
        if client == 'all':
    ...
    def list_clusterconfigs(self):
        return self.clusterconfigs
    ...
    def create_clusterconfig(self, clusterconfig, overrides={}, quiet=False):
        return self._create_yaml_file(clusterconfig, self.clusterconfigs, 'clusterconfig', overrides=overrides,
                                      quiet=quiet)

    def delete_clusterconfig(self, clusterconfig, quiet=False):
        return self._delete_yaml_object(clusterconfig, self.clusterconfigs, 'clusterconfig', quiet=quiet)

    def update_clusterconfig(self, clusterconfig, overrides={}, quiet=False):
        return self._update_yaml_file(clusterconfig, self.clusterconfigs, 'clusterconfig', overrides=overrides,
