...
class Kbaseconfig:
    def __init__(self, client=None, containerclient=None, debug=False, quiet=False, offline=False):
        ...
        defaults['vncpassword'] = default.get('vnc', kdefaults.VNCPASSWORD)
        ...
        self.vncpassword = options.get('vncpassword')
    ...
    def create_app_openshift(self, app, overrides={}, outputdir=None):
        ...
        if app in kdefaults.LOCAL_OPENSHIFT_APPS:
            ...
            name, catalog, channel, csv, description, namespace, own, channels, crds = common.olm_app(app, overrides)
            if name is None:
            ...
            app_data = {'catalog': catalog, 'channel': channel, 'namespace': namespace, 'csv': csv, 'own': own}
    ...
    def delete_app_openshift(self, app, overrides={}):
        ...
        if app in kdefaults.LOCAL_OPENSHIFT_APPS:
            ...
            if name is None:
                error(f"Couldn't find any app matching {app}. Skipping...")
                return 1
            app_data = {'catalog': catalog, 'channel': channel, 'namespace': namespace, 'crds': crds, 'own': own}
            app_data.update(overrides)
    ...
    def info_app_openshift(self, app, overrides={}):
        ...
        if app not in kdefaults.LOCAL_OPENSHIFT_APPS:
            name, catalog, defaultchannel, csv, description, namespace, own, channels, crds = olm_app(app, overrides)
            if name is None:
    ...
    def create_workflow(self, workflow, overrides={}, outputdir=None, run=True, basedir=None):
        ...
        for entry in files:
            ...
            if not os.path.exists(destdir):
                ...
                os.makedirs(destdir)
            # If we've already processed the directory tree then there's no point in looking at individual files.
