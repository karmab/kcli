...
def create(config, plandir, cluster, overrides):
    ...
    if kubevirt and safe_load(os.popen(kubevirt_crd_cmd).read()) is None:
        ...
        app_name, source, channel, csv, description, x_namespace, own, channels, crds = olm_app(app)
    ...
    if need_mce:
        ...
        app_name, source, channel, csv, description, x_namespace, own, channels, crds = olm_app(app)
        app_data = {'name': app_name, 'source': source, 'channel': channel, 'namespace': x_namespace, 'csv': csv,
                    'own': own, 'mce_hypershift': mce_hypershift, 'assisted': need_assisted,
                    'version': version, 'tag': tag, 'pull_secret': pull_secret}
        result = config.create_app_openshift(app_name, app_data)
        if result != 0:
