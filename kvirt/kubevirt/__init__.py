...
DOMAIN = "kubevirt.io"
VERSION = 'v1alpha1'
REGISTRYDISKS = ['alpine', 'cirros', 'fedora-cloud']
...
class Kubevirt(object):
    ...
    def info(self, name, output='plain', fields=None, values=False):
        ...
        if 'interfaces' in status:
            ...
            for interface in interfaces:
                if 'ipAddress' in interface:
                    ip = interface['ipAddress']
                    break
        yamlinfo = {'name': name, 'nets': [], 'disks': [], 'state': state, 'memory': memory, 'creationdate': creationdate, 'host': host}
        if ip is not None:
            yamlinfo['ip'] = ip
        if plan is not None:
            yamlinfo['plan'] = plan
        ...
        for d in plaindisks:
            ...
            if 'persistentVolumeClaim' in volumeinfo:
                _type = 'pvc'
                pvc = core.read_namespaced_persistent_volume_claim(volumename, namespace)
    ...
    def delete(self, name, snapshots=False):
        ...
        spec = vm.get("spec")
        volumes = [d['volumeName'] for d in spec['domain']['devices']['disks']]
    ...
    def list_disks(self):
        ...
        pvc = core.list_namespaced_persistent_volume_claim(namespace)
        for p in pvc.items:
            if 'kcli/template' in p.metadata.annotations:
                continue
            else:
