...
class Kproxmox(Kbase):
        ...
        for storage in all_storages:
            ...
            if storage_name in storages:
                ...
                storages[storage_name] = {
                    ...
                    "node": storage["node"],
                    "size": f"{sizeof_fmt(storage['maxdisk'] - storage['disk'])} free / {sizeof_fmt(storage['maxdisk'])} total",
                }
    ...
    def _parse_notes(self, notes):
        ...
        if notes:
            for line in notes.splitlines():
                match = re.search(f"^kvirt:(\w+): (.*)$", line)
                if match:
    ...
    def _set_cloudinit(
        ...
        self._wait_for(
            vm.config.post(
                cicustom=f"meta=local:snippets/{vmid}-cloudinit-metadata.yaml,user=local:snippets/{vmid}-cloudinit-userdata.yaml",
                ipconfig0=",".join(net0),
